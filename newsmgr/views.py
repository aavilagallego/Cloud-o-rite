...
from newsmgr.models import Links
...
def home(request):
    latest_news = Links.objects.all().order_by('-datep')
    print "I'm here"
...
def addnew(request):
    if request.method == 'POST':
        ...
        n = Links(url=form.cleaned_data['ilink'], title=form.cleaned_data['ititle'], taglist=form.cleaned_data['itags']);
        n.date = timezone.now();
        n.votes=1
        n.spam=0
        n.status='N'
